    ...
    return (
        ...
        <HeaderIndex navigation={props.navigation}/>
            ...
            <View style = {[componentStyles.container_v2,{alignItems: "center"}]}>
                <View style={{top:-50}}><NetPoint text={"You now have earned"} netpoint={netpoint}/></View>
            
   
            
            <FlatList style={{top:30}}
                data={sampleData}
                keyExtractor={({ id }) => id.toString()}
                renderItem={({ item }) => (
                    <View style={styles.giftContainer}>
                        <Text style={styles.giftNetPoint}>{item.netpoint} NET POINTS</Text>                  
                        <Text style={styles.giftTitle}>{item.title}</Text>
                        <Text style={styles.giftContent}>{item.description}</Text>
                        <TouchableOpacity style={styles.giftBtnContainer}
                            onPress={()=>{
                                console.log("clicked");
